...
def main():
    ...
    parser_scan_efficiency.add_argument(
        "config", type=pathlib.Path, help="Configuration file in YAML format"
    )
    parser_scan_efficiency.add_argument(
        "output", type=pathlib.Path, help="Output csv file"
    )
    parser_scan_efficiency.set_defaults(
        func=lambda args: ftm.commands.scans.efficiency_voltage.scan(
    ...
    parser_scan_efficiency.set_defaults(
        func=lambda args: ftm.commands.scans.threshold.scan(args.config, args.output)
    )
    ...
    parser_scan_efficiency.set_defaults(
        func=lambda args: ftm.commands.scans.efficiency_laser.scan(
            args.config, args.output
        )
    )
    ...
    parser_analyze_efficiency.add_argument(
        "input", type=pathlib.Path, help="Raw result of the efficiency scan"
    )
    parser_analyze_efficiency.add_argument(
        "output", type=pathlib.Path, help="Output of the analysis"
    )
    parser_analyze_efficiency.add_argument(
        "threshold", type=float, help="Threshold in mV"
    )
    parser_analyze_efficiency.add_argument(
        "--plot", type=pathlib.Path, help="Plot directory", default=None
    )
    parser_analyze_efficiency.set_defaults(
        func=lambda args: ftm.commands.analysis.efficiency_laser.analyze(
            args.input, args.output, args.threshold, args.plot
        )
    )
    # "analyze threshold" subcommand
    parser_analyze_threshold = analyze_subparsers.add_parser("threshold")
    parser_analyze_threshold.add_argument(
        "input", type=pathlib.Path, help="Result of the threshold scan"
    )
    parser_analyze_threshold.add_argument(
        "output", type=pathlib.Path, help="Output of the analysis"
    )
    parser_analyze_threshold.set_defaults(
        func=lambda args: ftm.commands.analysis.threshold.analyze(
    ...
    parser_analyze_currents.add_argument(
        "output", type=pathlib.Path, help="Output of the analysis"
    )
    parser_analyze_currents.add_argument(
        "--variable",
        type=str,
        default="att",
        help="Variable of the scan (att or vampl)",
    )
    parser_analyze_currents.add_argument(
        "--gain", type=pathlib.Path, default=None, help="Gain calibration file"
    )
    parser_analyze_currents.add_argument(
        "--plot",
        type=pathlib.Path,
        default=None,
        help="Folder where to plot the result",
    )
    parser_analyze_currents.set_defaults(
        func=lambda args: ftm.commands.analysis.currents.analyze_scan(
            args.input, args.output, args.variable, args.plot, args.gain
        )
    )
    ...
    parser_plot_efficiency.add_argument(
        "input", type=pathlib.Path, help="Analyzed result of the efficiency scan"
    )
    parser_plot_efficiency.add_argument(
    ...
    parser_import_scan.add_argument(
        "input", type=pathlib.Path, help="Input directory with current sweep data"
    )
    parser_import_scan.add_argument(
        "output", type=pathlib.Path, help="Output file path"
    )
    parser_import_scan.set_defaults(
        func=lambda args: ftm.commands.legacy.import_scan(args.input, args.output)
    )
    # "import gain" subcommand
    parser_import_gain = import_subparsers.add_parser("gain")
    parser_import_gain.add_argument(
        "input", type=pathlib.Path, help="Input gain ROOT file"
    )
    parser_import_gain.add_argument(
        "output", type=pathlib.Path, help="Output file path"
    )
    parser_import_gain.set_defaults(
        func=lambda args: ftm.commands.legacy.import_gain(args.input, args.output)
    )
