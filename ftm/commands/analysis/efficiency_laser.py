...
def plot(input_file, output_dir):
    ...
    for attenuation, df in attenuation_groups:
        ...
        result_dict["err_efficiency"].append(
            fired / total * np.sqrt(1 / fired + 1 / total)
        )
...
def analyze(input_file, result_dir, threshold, plot_dir=None):
    ...
    if plot_dir:
        os.makedirs(plot_dir / "signals", exist_ok=True)
    ...
    while True:
        """unpack event"""
        try:
            amplification, actual_attenuation, size_waveform_raw = struct.unpack_from(
                ">ffi", iraw, offset
            )
        except struct.error:
            break  # file finished, stop unpacking
        ...
        if plot_dir and i % 1000 == 0:
            waveform.save_figure(
                plot_dir / f"signals/{amplification}V_{actual_attenuation}att.png"
            )
        i += 1
    print("\nUnpacking completed.")
