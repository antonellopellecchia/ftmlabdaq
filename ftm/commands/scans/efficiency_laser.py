...
def scan(configuration_file, output_file):
    ...
    attenuator_controller = attenuator.Attenuator(
        setup["attenuator"]["host"], setup["attenuator"]["port"]
    )
    ...
    for amplification in ampl_range:
        ...
        for attenuation in att_range:
            ...
            for isignal in tqdm(range(nsignals)):
                ...
                waveform_raw = scope_controller.get_waveform_raw(
                    setup["measurement"]["signal"]
                )
                with open(output_file, "ab") as ostream:
                    ostream.write(
                        struct.pack(
                            ">ffi", amplification, actual_attenuation, len(waveform_raw)
                        )
                    )
